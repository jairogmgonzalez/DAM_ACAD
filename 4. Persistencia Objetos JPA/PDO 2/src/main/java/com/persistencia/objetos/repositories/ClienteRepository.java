...
public interface ClienteRepository extends JpaRepository<Cliente, Long> {
        ...
        // Busca los clientes cuya cantidad de productos comprados sea superior a una
        // cantidad dada
        @Query(value = "SELECT DISTINCT c.* FROM clientes c "
                        + "JOIN pedidos p ON p.cliente_id = c.id "
                        + "JOIN pedidos_productos pp ON pp.pedido_id = p.id "
                        + "WHERE pp.cantidad > :cantidad", nativeQuery = true)
        List<Cliente> findByCantidadCompradaSuperiorA(@Param("cantidad") Integer cantidad);

        // Busca los clientes cuya cantidad de productos comprados sea inferior a una
        // cantidad dada
